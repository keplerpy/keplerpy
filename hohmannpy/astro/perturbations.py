...
from . import propagation, logging, time
...
class SolarRadiation(Perturbation):
    ...
    def evaluate(self, time: float, state: np.ndarray) -> tuple[float, float, float]:
        ...
        position_sun_wrt_earth = -dcms.euler_2_dcm(earth_tilt, 1) @ self.earth_orbit_spline(time)
        position_sun_wrt_sat = position_sun_wrt_earth - state[:3]
